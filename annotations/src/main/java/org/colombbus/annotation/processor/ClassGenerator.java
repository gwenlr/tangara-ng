...
class ClassGenerator {
	 ...
	 */
	private String generateMethodCode(ExecutableElement method, Dictionary dictionary, LocalizableMethod localizableInfo)
			throws MissingResourceException {
	 ...
	 * @param method
	 *            the method to check
	 * @return <code>true</code> if the method us overridden, <code>false</code>
	 *         otherwise
	 */
	 ...
	 * @return an extract java code representing a set of methods definition or
	 *         an empty string
	 */
	private String generateInheritedCode(TypeElement classType, Locale locale) {
		...
		for (ExecutableElement method : methods) {
			...
			Entry<ExecutableElement, TypeElement> entry = new SimpleEntry<ExecutableElement, TypeElement>(method, classType);
		...
		if (localizeInfo.inherit()) {
			TypeElement superClassType = (TypeElement) typeUtils().asElement(classType.getSuperclass());
	 ...
	 * @throws MissingResourceException
	 */
	private void generateClassCode(TypeElement modelType, Locale locale) throws IOException, MissingResourceException {
		...
		try {
			FileObject javaClassFile = processingEnv.getFiler().createSourceFile(canonicalGenClassName);
			...
			String genAnnotCode = String.format(GENERATE_STMT, LocalizeProcessor.class.getCanonicalName(), Calendar.getInstance());
			...
			for (Element enclosedElem : modelType.getEnclosedElements()) {
				...
				if (kind == ElementKind.CONSTRUCTOR || kind == ElementKind.METHOD) {
					...
					LocalizableMethod localizableInfo = method.getAnnotation(LocalizableMethod.class);
					...
					if (kind == ElementKind.METHOD) {
						SimpleEntry<ExecutableElement, TypeElement> scannedMethod = new SimpleEntry<ExecutableElement, TypeElement>(method,
								modelType);
			...
			// Check if the superclass has to be scanned
			if (localizeInfo.inherit()) {
				TypeElement superType = (TypeElement) typeUtils().asElement(modelType.getSuperclass());
